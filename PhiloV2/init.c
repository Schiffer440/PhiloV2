...
/*   Updated: 2023/11/03 16:10:17 by adugain          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_prog(t_prog *prog, char **av)
{
	...
	prog->philo = malloc(sizeof(t_philo) * ft_atol(av[1]));
	prog->forks = malloc(sizeof(pthread_mutex_t) * ft_atol(av[1]));
	if (!prog->philo || !prog->forks)
		return (1);
	prog->death_time = ft_atol(av[2]);
	if (av[5])
		prog->meal_needed = ft_atol(av[5]);
	else
		prog->meal_needed = -1;
	prog->dead = 0;
...
int	init_philo_fork(t_prog *prog, t_philo *philo, char **av)
{
	...
	while (++i < ft_atol(av[1]))
	{
		...
		(prog->death_time - (ft_atol(av[3]) + ft_atol(av[4]))) / 2;
		philo[i].prog = prog;
		philo[i].id = i + 1;
		philo[i].eat_count = 0;
		philo[i].eating = 0;
		...
		if (i == 0)
			philo[i].r_fork = &philo[i].prog->forks[ft_atol(av[1]) - 1];
		else
